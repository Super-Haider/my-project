...
uint8_t capture_ready = 0;      // 0 = waiting for first edge, 1 = valid measurements
/* USER CODE END PV */
...
static void MX_GPIO_Init(void)
{
  ...
  /* PC6 → TIM3_CH1 Input Capture AF2 */
  GPIO_InitStruct.Pin       = GPIO_PIN_6;
  GPIO_InitStruct.Mode      = GPIO_MODE_AF_PP;
  GPIO_InitStruct.Pull      = GPIO_NOPULL;   // ← change to PULLDOWN if noisy
  GPIO_InitStruct.Speed     = GPIO_SPEED_FREQ_HIGH;
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
  if (htim->Instance == TIM3 && htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1)
  {
    ...
    {
      // 48 MHz × 1000 / period → milli-Hz (use 64-bit to avoid overflow)
